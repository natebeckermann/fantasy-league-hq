...
export default async function handler(req,res){
  try{
    const q=String(req.query.q||'2027 NFL Draft prospects').slice(0,120);
    const url='https://news.google.com/rss/search?q='+encodeURIComponent(`${q} college football NFL Draft`)+"&hl=en-US&gl=US&ceid=US:en";
    ...
    const items=[...xml.matchAll(/<item>([\s\S]*?)<\/item>/gi)].slice(0,8).map(m=>{
      const x=m[1];return{title:tag(x,'title'),link:tag(x,'link'),published:tag(x,'pubDate'),source:tag(x,'source')}
    }).filter(x=>x.title&&x.link);
    res.setHeader('Cache-Control','s-maxage=900, stale-while-revalidate=3600');
    res.status(200).json({query:q,items});
  }catch(e){res.status(500).json({error:'Could not load prospect news',items:[]})}
}
